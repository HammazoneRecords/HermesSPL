...
// Cron Activity Feed — read-only unified view of all cron runs.
export function getCronActivity(params?: {
  jobs?: string[]
  status?: string[]
  range?: number
  limit?: number
  cursor?: string
}): Promise<CronActivityResponse> {
  ...
  return hermesApi<CronActivityResponse>({
    ...profileScoped(),
    ...connectionScoped(),
    path: `/api/cron/activity${suffix}`,
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
}

export async function getCronActivityDetail(runId: string): Promise<CronRunActivity> {
  const { run } = await hermesApi<{ run: CronRunActivity }>({
    ...profileScoped(),
    ...connectionScoped(),
    path: `/api/cron/activity/${encodeURIComponent(runId)}`
  })
  return run
}
