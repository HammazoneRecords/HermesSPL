...
export function notifyCronChanged(): void {
  ...
  void import('@/store/cron-activity').then(({ $cronActivityUnreadTick }) => {
    $cronActivityUnreadTick.set($cronActivityUnreadTick.get() + 1)
  }).catch(() => {
    // cron-activity store may not be loaded
  })
}
