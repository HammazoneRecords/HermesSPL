...
import type { CronRunActivity } from '@/types/hermes'
...
export function CronView({ onClose, onOpenSession, setStatusbarItemGroup: _setStatusbarItemGroup }: CronViewProps) {
  ...
  // Activity tab toggle: 'jobs' | 'activity'
      ...
      {loading && jobs.length === 0 ? (
        ...
        <PanelBody>
          ...
          {activeTab === 'jobs' ? (
            ...
            <ActivityFeed jobs={jobs} onOpenSession={onOpenSession} />
          )}
...
function ActivityFeed({
  ...
  const [selectedRunId, setSelectedRunId] = useState<null | string>(null)
  const changeEventsAvailable = useStore($changeEventsAvailable)
  const cronChangeTick = useStore($cronChangeTick)

  const activityQuery = useQuery({
    queryKey: ['cron-activity', range, statusFilter],
    queryFn: () =>
      getCronActivity({
        range,
    ...
    refetchInterval: changeEventsAvailable ? 8000 : 60_000
  })

  const runs = activityQuery.data?.runs ?? []
  const byStatus = activityQuery.data?.byStatus ?? {}

  // Detail view for a selected run
  const [detail, setDetail] = useState<null | CronRunActivity>(null)
  useEffect(() => {
    if (!selectedRunId) {
      setDetail(null)
      return
    ...
    let cancelled = false
    getCronActivityDetail(selectedRunId)
      .then(run => {
    ...
    return (
      <div className="flex flex-col gap-2 p-2">
        <button
          ...
          type="button"
        >
          <Codicon name="arrow-left" className="size-3" />
          {c.backToFeed ?? 'Back to feed'}
        </button>
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <span className={`size-2 rounded-full ${STATUS_DOT[detail.status] ?? 'bg-gray-400'}`} />
            <span className="text-sm font-medium">{detail.jobName}</span>
            <PanelPill tone={detail.status === 'error' ? 'bad' : detail.status === 'ok' ? 'good' : 'muted'}>
              {STATUS_LABEL[detail.status] ?? detail.status}
            </PanelPill>
          </div>
          {detail.outputPreview && (
            <PanelBlock className="text-xs">
              <pre className="whitespace-pre-wrap font-mono">{detail.outputPreview}</pre>
            </PanelBlock>
          )}
          {detail.fullOutput && detail.fullOutput !== detail.outputPreview && (
            <PanelBlock className="text-xs">
              <pre className="whitespace-pre-wrap font-mono">{detail.fullOutput}</pre>
            </PanelBlock>
          )}
          {detail.errorSummary && (
            <div className="flex items-start gap-1.5 rounded bg-destructive/10 p-2 text-[0.7rem] text-destructive">
              <AlertTriangle className="mt-px size-3 shrink-0" />
              <span className="min-w-0 break-words">{detail.errorSummary}</span>
            </div>
          )}
          <PanelMeta
            rows={[
              { label: c.scheduledAt ?? 'Scheduled', value: formatTime(detail.scheduledAt) },
              { label: c.startedAt ?? 'Started', value: formatTime(detail.startedAt) },
              { label: c.finishedAt ?? 'Finished', value: formatTime(detail.finishedAt) },
              { label: c.deliverLabel, value: detail.deliveryTarget },
              { label: c.deliveryStatus ?? 'Delivery status', value: detail.deliveryStatus }
            ]}
          />
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col gap-2 p-2">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex items-center gap-1">
          {(['ok', 'error', 'running', 'unknown'] as const).map(status => (
            <button
              className={`rounded px-2 py-0.5 text-[0.65rem] font-medium transition-colors ${
      ...
      {activityQuery.isLoading ? (
        <PageLoader label={c.loading ?? 'Loading...'} />
      ) : runs.length === 0 ? (
        <PanelEmpty
          icon="inbox"
          title={c.noRuns ?? 'No runs in this period'}
          description={c.emptyActivityDesc ?? 'Cron runs will appear here.'}
        />
      ) : (
        <div className="flex flex-col gap-px">
          {runs.map(run => (
            <button
              className="flex items-start gap-2 rounded p-2 text-left transition-colors hover:bg-(--ui-bg-quinary)"
              key={run.runId}
              onClick={() => setSelectedRunId(run.runId)}
              type="button"
            >
              <span className={`mt-1.5 size-2 shrink-0 rounded-full ${STATUS_DOT[run.status] ?? 'bg-gray-400'}`} />
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2">
                  <span className="truncate text-xs font-medium">{run.jobName}</span>
                  <span className="shrink-0 text-[0.65rem] text-muted-foreground">
                    {relativeTime(Date.parse(run.scheduledAt))}
                  </span>
                </div>
                {run.outputPreview && (
                  <p className="mt-0.5 line-clamp-2 text-[0.7rem] text-muted-foreground">
                    {truncate(run.outputPreview, 120)}
                  </p>
                )}
                {run.errorSummary && (
                  <p className="mt-0.5 line-clamp-1 text-[0.7rem] text-destructive">
                    {truncate(run.errorSummary, 80)}
                  </p>
                )}
              </div>
            </button>
