...
def _save_compose_deliver(
    ...
    # EXCEPTION: section:cron (Phase 3) is activity-feed-only by explicit user
    # choice — [SILENT] must NOT suppress it or the user gets nothing.
    _is_section_cron = (
        _normalize_deliver_value(_delivery_lane_value(job, for_failure=not d.success)) == "section:cron"
    )
    if d.should_deliver and d.success and not _is_section_cron and _is_cron_silence_response(deliver_content):
    ...
    # section:cron (Phase 3): deliver to the activity feed only — no chat platform
    # send.  Output is already saved above via save_job_output.  Suppress the
    # actual _deliver_result call so nothing reaches a chat, but leave should_deliver
    # True's side effects (output file, last_output_preview) intact.
    if d.should_deliver:
        _lane_deliver = _normalize_deliver_value(_delivery_lane_value(job, for_failure=not d.success))
        if _lane_deliver == "section:cron":
            logger.info(
                "Job '%s': deliver=section:cron — skipping chat delivery "
                "(output saved to activity feed)", job["id"])
            d.should_deliver = False
