...
def save_job_output(job_id: str, output: str):
    """Save job output to file, persisting a preview snippet to jobs.json."""
    ensure_dirs()
    ...
    # Persist a plain-text preview (~200 chars) to jobs.json for the activity feed.
    _persist_output_preview(job_id, output)
    return output_file


def _persist_output_preview(job_id: str, output: str) -> None:
    """Write a truncated plain-text preview to the job record for the activity feed."""
    if not output:
        return
    # Strip common markdown noise, take first non-empty line, cap at 200 chars.
    preview = output[:300].split("\n")[0].strip()[:200]
    if not preview:
        return

    def apply(jobs, _i, job):
        job["last_output_preview"] = preview
        save_jobs(jobs)

    try:
        _with_job(job_id, apply, None)
    except Exception:
        logger.debug("Failed to persist output preview for job %s", job_id)
