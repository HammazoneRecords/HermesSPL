...
def cron_delivery_targets() -> list[dict]:
    ...
    prepend the implicit ``local`` option themselves. ``section:cron`` (activity feed only) is
    included as a virtual target so it surfaces in the UI checkbox group."""
    targets: list[dict] = [
        {"id": "section:cron", "name": "Activity feed only", "home_target_set": True, "home_env_var": None},
    ]
    try:
...
def _resolve_single_delivery_target(
    ...
    if deliver_value == "local":
        return None
    # section:cron = activity feed only (no chat delivery).  Treated as a no-op
    # target like `local`, but the job's `deliver` field stays `section:cron` so the
    # activity feed can render it distinctly (see cron-ui-section.md Phase 3).
    if deliver_value == "section:cron":
        return None
...
def _unresolved_delivery_outcome(job: dict, for_failure: bool) -> Optional[str]:
    ...
    ``section:cron`` (activity feed only — no chat target), and origin-less ``origin`` (CLI jobs
    never capture an origin — a spurious error every run), else an error string."""
    deliver_value = _normalize_deliver_value(_delivery_lane_value(job, for_failure=for_failure))
    if deliver_value == "local":
        return None
    # section:cron delivers to the activity feed only — not resolving a chat target is expected.
    if deliver_value == "section:cron":
        return None
    if deliver_value == "origin":
        logger.info(
