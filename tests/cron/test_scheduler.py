...
class TestCronDeliveryTargets:
    ...
    def test_lists_configured_platforms_flagging_missing_home_channel(self, monkeypatch):
        ...
        # bot-chat:<profile> entries (machine-local Bot Chat injection) and the
        # virtual section:cron target (activity feed only) ride the same listing
        # but are not gateway platforms — scope the platform assertions to the
        # gateway entries.
        platform_targets = {k: v for k, v in targets.items()
                           if not k.startswith("bot-chat") and k != "section:cron"}
