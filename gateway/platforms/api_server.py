...
try:
    ...
    from cron.activity import list_activity_runs as _cron_activity_list, get_run_detail as _cron_activity_detail
except ImportError:
    _cron_list = _cron_get = _cron_create = _cron_update = None
    _cron_remove = _cron_pause = _cron_resume = _cron_trigger = None
    _cron_activity_list = _cron_activity_detail = None
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    def _http_route_table(self) -> List[tuple]:
        ...
        if _CRON_AVAILABLE:
            ...
            routes.append(("GET", "/api/cron/activity", self._handle_cron_activity))
            routes.append(("GET", "/api/cron/activity/{run_id}", self._handle_cron_activity_detail))
        return routes
    ...
    async def _handle_cron_activity(self, request: "web.Request") -> "web.Response":
        """GET /api/cron/activity — read-only activity feed of all cron runs."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        if not _CRON_AVAILABLE or _cron_activity_list is None:
            return web.json_response({"error": "Cron activity not available"}, status=501)
        try:
            jobs_param = request.query.get("jobs")
            jobs_filter = [j.strip() for j in jobs_param.split(",") if j.strip()] if jobs_param else None

            status_param = request.query.get("status")
            status_filter = [s.strip() for s in status_param.split(",") if s.strip()] if status_param else None

            range_hours = int(request.query.get("range", "24"))
            limit = min(int(request.query.get("limit", "100")), 500)
            cursor = request.query.get("cursor") or None

            result = _cron_activity_list(
    ...
    async def _handle_cron_activity_detail(self, request: "web.Request") -> "web.Response":
        """GET /api/cron/activity/{run_id} — single run detail with full output."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        if not _CRON_AVAILABLE or _cron_activity_detail is None:
            return web.json_response({"error": "Cron activity not available"}, status=501)
        try:
            run_id = request.match_info["run_id"]
            detail = _cron_activity_detail(run_id)
            if detail is None:
                return web.json_response({"error": "Run not found"}, status=404)
            return web.json_response({"run": detail})
        except Exception as e:
            return self._cron_error_response(e)
